...
function Home() {
  ...
  const [allProductsFromCart, setAllProductsFromCart] = useState([]);
  const [allProducts, setAllProducts] = useState([]);
  const [filteredArr, setFilteredArr] = useState([]);

  useEffect(() => {
    axios.get(`/products/prod`).then((res1) => {
      setAllProducts(res1.data);
      console.log(res1.data);
      axios
        .get(`/api/getAllProductsFromCart/ala.hamadi.01@gmail.com`)
        .then((res2) => {
          console.log(res2.data);

          setAllProductsFromCart(res2.data);
          const filteredArr1 = res1.data.filter((item) =>
            res2.data.includes(item._id)
          );
